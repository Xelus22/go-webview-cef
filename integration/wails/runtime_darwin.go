...
func installDispatchers() {
	dispatchInitOnce.Do(func() {
		cefshim.SetMessageDispatch(func(browser cefbindings.BrowserHandle, name, payload string, userData uintptr) {
			rt := currentRuntime()
			if rt == nil {
				return
			}
			rt.dispatchMessage(browser, name, payload, userData)
		})
		cefshim.SetLoadDispatch(func(browser cefbindings.BrowserHandle, success bool, userData uintptr) {
			rt := currentRuntime()
			if rt == nil {
				return
			}
			rt.dispatchLoad(browser, success, userData)
		})
		cefshim.SetCloseDispatch(func(browser cefbindings.BrowserHandle, userData uintptr) {
			rt := currentRuntime()
			if rt == nil {
				return
			}
			rt.dispatchClose(browser, userData)
		})
		cefshim.SetRequestDispatch(func(browser cefbindings.BrowserHandle, req cefshim.Request, userData uintptr) (cefshim.RequestResponse, bool) {
			rt := currentRuntime()
			if rt == nil {
				return cefshim.RequestResponse{}, false
			}
			return rt.dispatchRequest(browser, req, userData)
		})
	})
}
...
func (r *Runtime) CreateWindow(opts WindowOptions) (*Window, error) {
	...
	cefshim.SetCloseCallbackUserData(uintptr(handle))
	cefshim.SetRequestCallbackUserData(uintptr(handle))
...
func (r *Runtime) dispatchMessage(browser cefbindings.BrowserHandle, name, payload string, userData uintptr) {
	lookupHandle := browser
	if userData != 0 {
...
func (r *Runtime) dispatchLoad(browser cefbindings.BrowserHandle, success bool, userData uintptr) {
	lookupHandle := browser
	if userData != 0 {
		lookupHandle = cefbindings.BrowserHandle(userData)
	}
...
func (r *Runtime) dispatchClose(browser cefbindings.BrowserHandle, userData uintptr) {
	lookupHandle := browser
	if userData != 0 {
		lookupHandle = cefbindings.BrowserHandle(userData)
	}
...
func (r *Runtime) dispatchRequest(browser cefbindings.BrowserHandle, req cefshim.Request, userData uintptr) (cefshim.RequestResponse, bool) {
	lookupHandle := browser
	if userData != 0 {
		lookupHandle = cefbindings.BrowserHandle(userData)
	}
	...
	request := Request{
		Method:  req.Method,
		URL:     req.URL,
		Headers: decodeHeaderJSON(req.HeadersJSON),
		Body:    append([]byte(nil), req.Body...),
	}

	response, handled := cb(request)
	if !handled {
		return cefshim.RequestResponse{}, false
	}
	...
	return cefshim.RequestResponse{
		StatusCode:  response.StatusCode,
		StatusText:  response.StatusText,
		MimeType:    mimeType,
		HeadersJSON: encodeHeaderJSON(response.Headers),
